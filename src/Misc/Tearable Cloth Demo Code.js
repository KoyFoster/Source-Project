...
Point.prototype.update = function (delta) {
  ...
  delta *= delta;
  settings.nx = this.x + (this.x - this.px) * 0.99 + (this.vx / 2) * delta;
  settings.ny = this.y + (this.y - this.py) * 0.99 + (this.vy / 2) * delta;
...
Point.prototype.resolve_constraints = function () {
  ...
  this.x > settings.boundsx
    ? (this.x = 2 * settings.boundsx - this.x)
    : 1 > this.x && (this.x = 2 - this.x);
  this.y < 1
    ? (this.y = 2 - this.y)
    : this.y > settings.boundsy && (this.y = 2 * settings.boundsy - this.y);
};
...
var Cloth = function () {
  ...
  var start_x =
    settings.canvas.width / 2 - (settings.cloth_width * settings.spacing) / 2;

  for (var y = 0; y <= settings.cloth_height; y++) {
    for (var x = 0; x <= settings.cloth_width; x++) {
      var p = new Point(
        start_x + x * settings.spacing,
        settings.start_y + y * settings.spacing,
      );

      x !== 0 && p.attach(this.points[this.points.length - 1]);
      y === 0 && p.pin(p.x, p.y);
      y !== 0 &&
        p.attach(this.points[x + (y - 1) * (settings.cloth_width + 1)]);
