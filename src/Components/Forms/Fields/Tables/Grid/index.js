  ...
  const InitDataArray = (dataValue) => {
    ...
    const DataParse = () => {
      ...
      if (dataType === 'isString') {
        ...
        const DataRows = dataValue.split(
          (dataValue.indexOf(colDELIM) > -1 ? colDELIM : colDELIM2) +
            (dataValue.indexOf(rowDELIM) > -1 ? rowDELIM : rowDELIM2),
        );
  ...
  const captionButton = [
    ...
    <button
      ...
      style={{
        ...
        background:
          trueValue.length ===
            (funcs.getSelection ? funcs.getSelection().y : 0) ||
          (funcs.getSelection ? funcs.getSelection().y : 0) === 0
            ? '#444444'
            : captStyle.background,
      }}
