...
var Calc = {
  ...
  fMinMax(val, min, max) {
    if (min !== undefined) if (val < min) return min;
    if (max !== undefined) if (val > max) return max;
    return val;
  },
...
var Deg = {
  ...
  V2Normalize(Vector) {
    ...
    //check magnitude
    if (!fMag) {
      //if magnitude is zero, end fucntion
      return;
    }
    //normalize
    Vector.x /= fMag;
  ...
  fGetYawAngle(Self, Target) {
    //get yaw angle
    var fDistance = Math.sqrt(
      Math.pow(Self.z - Target.z, 2) + Math.pow(Self.x - Target.x, 2),
    );
    var fAngle = Math.atan2(Self.y - Target.y, fDistance);
    return -Deg.fNormalizeRadians(fAngle);
  },

  fDownToZero(Value, Range) {
...
var Coll = {
  ...
  b2DBoxToCircleTest(aMin, aMax, CirclePos, Radius) {
    ...
    var BR = new Vector2();
    TL.New(aMin.x, aMax.y);
    BR.New(aMax.x, aMin.y);
  ...
  bRayCircleTest(Ray, RayPos, CirclePos, Radius) {
    ...
    //Get Max
    var v2Radius = new Vector2();
    v2Radius.New(-Radius / 2, Radius / 2);
    var TempCircle = Vector2(CirclePos);
    Calc.vAdd(TempCircle, v2Radius);
    var fMaxAngle = Deg.fGetPitchAngle(RayPos, TempCircle);
    //Get Min
    v2Radius.New(Radius / 2, -Radius / 2);
    TempCircle.New(CirclePos);
    Calc.vSub(TempCircle, v2Radius);
