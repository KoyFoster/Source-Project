...
const Stocks = (props) => {
  ...
  const TEST_MODE = 'STOCK'; // STOCK or PRICES
  ...
  const Stock = ({ tag }) => {
    ...
    const [hideTrades, setHideTrades] = useState(true);
    ...
    const addUpdate = (d) => {
      if (d === undefined) {
        console.warn('No Data returned from the stock market');
        return;
      }

      // Array Check: Otherwise handle singlur object
      if (Array.isArray(d)) {
        const buffer = d.map((obj) => {
      ...
      // null check
      if (d.high === null) return; // I"m currently wondering what this means
      if (d.date === getLastData().date && d.minute === getLastData().minute)
        return;

      const today = new Date();

      data.push(formatData(d));
      setData([...data]);
      if (30 < data.length) data.shift();
    };

    // Note: It also doesn't do after hours
    const callAPI = (justLast = true) => {
      // Get Fetched API Data
      stock.latestPrice(tag, addUpdate, justLast);
    };
    ...
    useEffect(() => {
      // Init Ticker
      callAPI(false);
      setTicker(new Ticker(callAPI, 10000));
    }, []);
    ...
    let LINES2 = null;
    if (TEST_MODE === 'PRICES')
      LINES1 = data.length > 0 ? RTGTools.getItems(data[0], ['LINE']) : null;
    else if (TEST_MODE === 'STOCK') {
      LINES1 =
        data.length > 0
          ? RTGTools.getItems(
              { open: 0, high: 0, low: 0, average: 0, close: 0 },
              ['LINE'],
              keyColors,
            )
          : null;
      LINES2 =
        data.length > 0
          ? RTGTools.getItems({ trades: 0, volume: 0 }, ['LINE'], keyColors)
          : null;
    }
    ...
    return (
      <div key={tag}>
        ...
        <Card>
          <header>AMOUNTS</header>
          <Card style={{ display: 'flex', width: '512px', height: '256px' }}>
            <ResponsiveContainer>
              <LineChart syncId="stock" type={'LINE'} data={data}>
                <Tooltip key="Tooltip" />
                <Legend key="Legend" />
                ...
                />
                <XAxis
                  dataKey="label" // The key define here will pull from the data
                  label=""
                />
                {LINES1}
              </LineChart>
          ...
          <header
            style={{
              filter: hideTrades ? 'brightness(33%)' : 'brightness(100%)',
              background: hideTrades ? '#00000099' : '00000000',
            }}
  ...
  const getStocks = () => {
    return STOCKS.map((key) => <Stock key={key} tag={key}></Stock>);
  };

  // console.log("LINES:", LINES1, LINES2);
  // console.log("data:", data);
  return <div className="Stocks">{getStocks()}</div>;
};
